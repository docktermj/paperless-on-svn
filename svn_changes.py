...
if __name__ == "__main__":
    ...
    for svn_path in svn_paths:
        ...
        old_revision = svn_path.get("revision")

        repository_path = os.path.join(input_path, path)
        repository = svn.local.LocalClient(repository_path)
        info = repository.info()
        new_revision = info.get("entry_revision")
        svn_path["revision"] = new_revision

        # Determine the files that differ between the old and current revisions.

        diffs = repository.diff_summary(old_revision, new_revision)
